...
<div>
    <div class="bg-paper border-b border-line">
        <div class="max-w-[1216px] mx-auto px-8 py-16">
            ...
            <h1 class="text-[42px] font-bold tracking-[-0.5px] mb-[18px] text-pretty leading-tight">{{ $page->get('hero_title') }}</h1>
        ...
        @if ($partners->isNotEmpty())
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-[26px]">
                @foreach ($partners as $partner)
                    @php
                        $box = 'border border-line h-[130px] flex items-center justify-center bg-[#FBFAF8] p-4';
                    @endphp
                    @if ($partner->url)
    ...
    <div class="bg-ink">
        <div class="max-w-[1216px] mx-auto px-8 py-16 flex justify-between items-center gap-8 flex-wrap">
            <div>
                <div class="text-[26px] font-bold text-white mb-2.5 tracking-[-0.3px]">{{ $page->get('join_title') }}</div>
