...
<div>
    <div data-vt="hero" class="bg-paper border-b border-line">
        <div class="max-w-3xl mx-auto px-5 sm:px-8 py-10 lg:py-16">
            <a href="{{ route($loc.'.news') }}" wire:navigate class="text-sm font-bold tracking-widest text-brand mb-4 inline-block">← {{ __('Новини') }}</a>
            <div class="text-sm text-faint mb-3">{{ $article->published_at?->translatedFormat('j F Y') }}</div>
            <h1 class="text-3xl sm:text-4xl lg:text-4xl font-bold tracking-tight leading-tight text-pretty">{{ $article->tr('title') }}</h1>
        </div>
    </div>

    <article class="max-w-3xl mx-auto px-5 sm:px-8 py-10 lg:py-16">
        @if ($article->imageUrl())
        ...
        @if ($article->tr('excerpt'))
            <p class="text-lg leading-relaxed text-ink-soft font-medium mb-8">{{ $article->tr('excerpt') }}</p>
        @endif
        <div class="rich text-lg text-body leading-relaxed">{!! $article->tr('body') !!}</div>
    </article>

    @if ($more->isNotEmpty())
        <div class="bg-paper border-t border-line">
            <div class="max-w-7xl mx-auto px-5 sm:px-8 py-10 lg:py-16">
                <h2 class="text-2xl font-bold mb-8">{{ __('Още новини') }}</h2>
