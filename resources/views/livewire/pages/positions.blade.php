...
<div>
    ...
    <div class="bg-paper border-t border-line">
        <div class="max-w-[1216px] mx-auto px-8 py-16">
            <h2 class="text-3xl font-bold mb-[34px]">{{ $page->get('list_title') }}</h2>
            @if ($positions->isNotEmpty())
                <div class="bg-white border border-line">
                    @foreach ($positions as $p)
                        <div class="flex gap-7 px-[30px] py-6 items-center flex-wrap hover:bg-[#FBFAF8] {{ ! $loop->last ? 'border-b border-line' : '' }}">
