...
<div>
    <div class="bg-paper border-b border-line">
        <div class="max-w-[1216px] mx-auto px-8 py-16">
            ...
            <h1 class="text-[42px] font-bold tracking-[-0.5px] mb-[18px] max-w-[900px] text-pretty leading-tight">{{ $page->get('hero_title') }}</h1>
    ...
    <div class="max-w-[1216px] mx-auto px-8 py-16 grid lg:grid-cols-[1fr_1.6fr] gap-16">
        <h2 class="text-3xl font-bold tracking-[-0.3px]">{{ $page->get('goal_title') }}</h2>
        <div class="rich text-[16.5px] text-body">{!! $page->get('goal_body') !!}</div>
    </div>

    <div class="bg-paper border-y border-line">
        <div class="max-w-[1216px] mx-auto px-8 py-16">
            <h2 class="text-3xl font-bold mb-[34px]">{{ $page->get('pillars_title') }}</h2>
            <div class="grid lg:grid-cols-3 gap-[26px]">
                @foreach ($page->list('pillars') as $p)
                    <div class="bg-white border border-line px-[30px] pt-[30px] pb-[34px]">
    ...
    @if ($team->isNotEmpty())
        <div class="max-w-[1216px] mx-auto px-8 py-16">
            <h2 class="text-3xl font-bold mb-2.5">{{ $page->get('team_title') }}</h2>
            <p class="text-[15.5px] text-muted mb-[34px]">{{ $page->get('team_subtitle') }}</p>
            <div class="grid lg:grid-cols-3 gap-[26px]">
                @foreach ($team as $member)
                    <div class="border border-line">
                        @if ($member->photo)
                            <img src="{{ asset('storage/'.$member->photo) }}" alt="{{ $member->name }}" class="h-[300px] w-full object-cover">
                        @else
    ...
    <div class="bg-paper border-t border-line">
        <div class="max-w-[1216px] mx-auto px-8 py-[72px] grid lg:grid-cols-[1fr_1.6fr] gap-16">
            <h2 class="text-3xl font-bold tracking-[-0.3px] text-pretty">{{ $page->get('hub_title') }}</h2>
